...
function xml_serialize($arr, $htmlon = FALSE, $isnormal = FALSE, $level = 1) {
	$s = $level == 1 ? "<?phpxml version=\"1.0\" encoding=\"ISO-8859-1\"?>\r\n<root>\r\n" : '';
	...
	function innerparse(&$data) {
		...
		foreach ($sxe->children() as $second_gen) {
			...
			else
			{
				...
				if(isset($attributes['id']))
				{
					$xmldata = $second_gen->asXML();
					if(empty($xmldata))
					{
						$xml_array[$attributes['id']]=array();
					}
					else
					{
						$xml_array[$attributes['id']] = $this->innerparse($xmldata);
					}
