...
require_once UC_ROOT.'./model/sae_memcache_wrapper1.php';
...
class base {
	...
	function init_input($getagent = '') {
		...
		if($input) {
			...
			//} else
			if($this->time - $this->input('time') > 3600) {
	...
	function &cache($cachefile) {
		...
		if(!isset($_CACHE[$cachefile])) {
			...
			$cache_file = 'ucenter/cache/'.$cachefile.'.php';	
			$mmc = memcache_init();
			if($mmc==false)
			{
		   	echo "mc init failed\n";
			...
			{
				$cachecontent =  memcache_get($mmc,$cache_file);
			}
			//$s = new SaeStorage();	
